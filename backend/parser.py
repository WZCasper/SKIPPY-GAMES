...
def fetch_dlc_upsells(dlc_ids, uah_to_rub):
    ...
    for dlc_id in dlc_ids[:12]:  # ограничиваем, чтобы не раздувать время прогона
        ...
        base_price_rub = round(price_uah * uah_to_rub)
        price_rub = base_price_rub + MARKUP_RUB
        assert price_rub == base_price_rub + MARKUP_RUB  # наценка — строго +500, без исключений
        upsells.append({
            "id": dlc_id,
...
def fetch_appdetails(appid):
    ...
    return {
        ...
        "price_uah_initial": round(price_uah_initial, 2) if price_uah_initial is not None else None,
...
def build_game_record(appid, uah_to_rub):
    ...
    # --- Расчёт цены: строго Final = Base + 500, без исключений ---------
    # Бесплатные (free-to-play) игры остаются бесплатными (0 + 0 = 0) — им
    # не назначается наценка за доставку, так как покупать в них нечего.
    # Для всех платных игр: base_price_rub — переведённая в рубли цена из
    # Steam, markup_rub — фиксированная наценка ровно 500 ₽, price_rub —
    # их сумма. Assert ниже гарантирует, что наценка никогда не "поедет"
    # из-за будущих правок кода.
    if details["is_free"]:
        base_price_rub = 0
        markup_rub = 0
        price_rub = 0
        ...
        base_price_rub = round(details["price_uah"] * uah_to_rub)
        markup_rub = MARKUP_RUB
        price_rub = base_price_rub + markup_rub
        original_price_rub = None
        if details.get("discount_percent") and details.get("price_uah_initial") is not None:
            original_price_rub = round(details["price_uah_initial"] * uah_to_rub) + markup_rub
    assert price_rub == base_price_rub + markup_rub
        ...
        "games": [
            {
                ...
                "original_price_rub": g.get("original_price_rub"),
